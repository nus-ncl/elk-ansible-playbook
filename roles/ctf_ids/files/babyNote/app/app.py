...
@app.route('/flag')
def flag():
	flag="DSO-CTF{server_side_request_forgery_is_dangerous}"
	if is_inner_ipaddress(request.remote_addr):
		content=flag
	else:
		content='only localhost can get the flag'
	data = {
				'title': 'only localhost can get the flag',
				'text': content
			}
	return render_template('note.html', data=data)

@app.route('/y0u_n3v3r_gu3ss_1t/', methods=['GET'])
def y0u_n3v3r_gu3ss_1t():
	url = request.args['url']
	if not url:
		content="please tell me the remote note url"
	else:
		content = GetRemoteNote(url)
	data = {
			'text': content,
			'title': "Remote Note Content"
			}
	return render_template('note.html', data=data)
